...
export function extForAttachment(mediaType?: string, name?: string): string {
  ...
  const map: Record<string, string> = {
    ...
    'image/svg+xml': 'svg', 'application/pdf': 'pdf', 'text/markdown': 'md', 'text/plain': 'txt',
  }
...
function fileCard(asset: ExportAsset | undefined, name: string, mediaType: string): string {
  const label = name || (asset && asset.name) || 'file'
...
function renderBlock(b: CleanBlock, assets: ExportAssets): string {
  if (b.type === 'text') return `<div class="text">${mdToHtml(b.text)}</div>`
  if (b.type === 'thinking') {
  ...
  if (b.type === 'tool_result') {
    ...
    const imgs = (b.files || []).map(f => f ? assets.byName[f] : undefined).filter((a): a is ExportAsset => !!a && a.isImage)
      .map(a => imgTag(a, a.name)).join('')
  ...
  if (b.type === 'image') {
    ...
    if (a && a.isImage) return `<figure class="att">${imgTag(a, b.name || a.name)}<figcaption>${esc(b.name || a.name)}</figcaption></figure>`
...
export function renderChatViewerHtml(clean: CleanExport, assets: ExportAssets): string {
  const rows = clean.messages.map(msg => {
    ...
    const blocks = (msg.content || []).map(b => renderBlock(b, assets)).filter(Boolean).join('\n')
...
<title>${esc(clean.title)} &mdash; DataMoat export</title>
<style>
  :root{
  ...
  *{box-sizing:border-box}
  ...
  .promo{display:flex;gap:8px;align-items:center;justify-content:center;
    padding:7px 14px;font-size:12.5px;color:var(--muted2);
  ...
  header.doc h1{font-size:23px;margin:0 0 10px;font-weight:650;letter-spacing:-.2px}
  .doc-meta{display:flex;flex-wrap:wrap;gap:6px 16px;font-size:12.5px;color:var(--muted)}
  ...
  .role{font-weight:650;font-size:13.5px}
  ...
  .md-h{font-size:15px;margin:1em 0 .3em;color:var(--text)}
  ...
  pre.code{font-family:var(--mono);font-size:12.5px;line-height:1.5;background:var(--panel);
    border:1px solid var(--border);border-radius:8px;padding:12px 14px;overflow:auto;margin:.6em 0}
  pre.code code{white-space:pre}
  details.thinking{margin:.6em 0;border:1px dashed var(--border2);border-radius:8px;
    background:rgba(255,255,255,.015)}
  details.thinking summary{cursor:pointer;padding:8px 12px;color:var(--muted2);font-size:12.5px;
    user-select:none;font-weight:600}
  details.thinking[open] summary{border-bottom:1px dashed var(--border2)}
  details.thinking .text{padding:10px 14px;color:var(--muted2);font-size:13.5px}
  .tool-card{border:1px solid var(--border);border-radius:8px;margin:.7em 0;overflow:hidden;background:var(--panel)}
  .tool-card.result{background:rgba(116,182,165,.04)}
  .tool-head{padding:8px 13px;font-size:12.5px;color:var(--muted2);background:var(--panel2);
    border-bottom:1px solid var(--border)}
  ...
  figure.att figcaption{font-size:11.5px;color:var(--muted);margin-top:5px}
  .file-card{display:flex;align-items:center;gap:12px;text-decoration:none;color:inherit;
    border:1px solid var(--border);border-radius:9px;padding:11px 14px;margin:.7em 0;background:var(--panel);
  ...
  .file-open{color:var(--accent);font-size:12.5px;flex:none}
  ...
  <div class="wrap">
    ...
    ${rows}
    <footer class="doc">
      Generated by <a href="https://datamoat.org" target="_blank" rel="noopener">DataMoat</a> &middot; this is a clean, self-contained copy of your conversation.<br>
      The matching <code>chat.json</code> can be re-imported into DataMoat.
    </footer>
  </div>
</body></html>`
}
